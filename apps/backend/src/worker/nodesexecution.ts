...
async function updateExecution(
  ...
  const newTasksDone = (exec.tasksDone ?? 0) + 1;
  const totalTasks = exec.totalTasks ?? 0;
          ...
          if (executionId) {
            ...
            if (exec) {
              await prisma.execution.update({
                ...
                data: {
                  status: "PAUSED",
                  pausedNodeId: (job as any).data?.nodeId,
                },
              });
              console.log(
      ...
      try {
        ...
        if (workflowId && Array.isArray((job as any).data?.connections) && (job as any).data?.connections.length > 0) {
          ...
          if (workflow) {
            ...
            const nodes = (workflow.nodesJson as any) ?? (workflow.node as any) ?? {};
